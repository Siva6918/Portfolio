...
import { Cpu, Play, RefreshCw, Sparkles, Tag, ShieldAlert, RotateCcw } from 'lucide-react';
import { buildApiUrl } from '../../services/api';

const NLP_QUERY_ENDPOINT = buildApiUrl('/nlp/query');

const NlpEngineVisualizer = ({ isActive = true, className = '' }) => {
  ...
  // Automatically reset when moving away to another card
  useEffect(() => {
    if (!isActive) {
      handleReset();
    }
    ...
    <div className={`editorial-card p-5 sm:p-8 space-y-6 flex flex-col justify-between min-h-[540px] sm:min-h-[480px] h-full ${className}`}>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {(aiPrompt || fullAnswer || errorMsg) && (
            <button
              onClick={handleReset}
              className="flex items-center gap-1 text-[11px] font-mono text-slate-500 hover:text-rose-500 dark:text-white/50 dark:hover:text-rose-400 transition-colors"
              title="Reset prompt and output"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-xs font-semibold transition-all disabled:opacity-50 shadow-md active:scale-95 shrink-0"
            >
            ...
            <button
              onClick={handleReset}
              title="Reset NLP query and results"
              className="flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-slate-200 dark:bg-zinc-800 hover:bg-slate-300 dark:hover:bg-zinc-700 text-slate-700 dark:text-zinc-300 font-mono text-xs font-semibold transition-all active:scale-95 shrink-0"
            >
      ...
      {/* Error Alert Box */}
      {errorMsg && (
        <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-mono flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleReset}
            className="text-[10px] underline hover:text-rose-300 transition-colors"
          >
            Clear
          </button>
        </div>
